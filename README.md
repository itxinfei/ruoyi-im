...
> **项目重要声明**：
> 1. 本项目专注于 **桌面端浏览器** 体验，不支持移动端适配。
> 2. 请勿设计任何针对移动端的样式（Media Queries）或交互逻辑。
> 3. 所有界面设计应基于 1280px+ 宽度的桌面视口进行优化。
...
|模块|说明|技术栈|默认端口|核心职责|
|---|---|---|---|---|
|im-api|核心后端 API 服务|Spring Boot 2.7, WebSocket, Netty, MyBatis Plus|9999|用户认证、消息收发、好友/群组管理、文件传输核心逻辑、管理后台接口|
...
- 操作系统：开发/测试环境支持 Windows 10/11、CentOS 7.9；生产环境推荐 CentOS 7.9（64位），关闭 SELinux，开放 9999、6379 端口
...
3. 配置 WebSocket/Netty：检查 application.yml 中通信相关配置（默认无需修改，按需调整端口）：
        ...
        im:
          websocket:
            port: 9999  # WebSocket 绑定端口（与 API 服务端口一致）
          netty:
...
const service = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL || 'http://localhost:9999', // 后端 API 地址
  timeout: 5000
})
```
...
```javascript
const ws = new WebSocket(`ws://localhost:9999/im/websocket/${userId}`)
```
