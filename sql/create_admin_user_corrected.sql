...
INSERT INTO `im_user` (`id`, `username`, `password`, `nickname`, `avatar`, `gender`, `mobile`, `email`, `status`, `signature`, `last_online_time`, `create_time`, `update_time`)
...
-- 验证数据
SELECT '=== 用户1信息 ===' as info;
SELECT id, username, nickname, avatar FROM `im_user` WHERE id = 1;

SELECT '=== 用户1的会话列表 ===' as info;
SELECT
    cm.conversation_id,
    c.type,
    c.name,
